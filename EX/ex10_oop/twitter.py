...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    for tweet in tweets:
        if (
                tweet.retweets / tweet.time
                > fastest_growing.retweets / fastest_growing.time
        ):
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    hashtags = {}
    for tweet in tweets:
        for hashtag in tweet.content.split():
            if hashtag.startswith("#"):
                if hashtag in hashtags:
                    ...
                    hashtags[hashtag] = tweet.retweets
    return sorted(
        hashtags, key=lambda hashtag: (hashtags[hashtag], hashtag), reverse=True
    )


if __name__ == "__main__":
    tweet1 = Tweet(
        "@realDonaldTrump", "Despite the negative press covfefe #bigsmart", 1249, 54303
    )
    tweet2 = Tweet(
        "@elonmusk", "Technically, alcohol is a solution #bigsmart", 366.4, 166500
    )
    tweet3 = Tweet(
        "@CIA",
        "We can neither confirm nor deny that this is our first tweet. #heart",
        2192,
        284200,
    )
