...
def read_from_file_into_list(filename: str) -> list:
    ...
    with open(filename) as file:
        ...
        clients.extend(
            Client(row[0], row[1], int(row[2]), int(row[3]), int(row[4]))
            for row in reader
        )
...
def largest_earnings_per_day(filename: str) -> Optional[Client]:
    ...
    clients = read_from_file_into_list(filename)
    best_client = None
    best_client_earnings = sorted(
        [client.earnings_per_day() for client in clients], reverse=True
    )[0]
    if best_client_earnings <= 0:
        return None
    for client in clients:
        if client.earnings_per_day() == best_client_earnings and (
            best_client is None or client.account_age < best_client.account_age
        ):
            best_client = client
    return best_client
...
def largest_loss_per_day(filename: str) -> Optional[Client]:
    ...
    clients = read_from_file_into_list(filename)
    worst_client = None
    worst_client_loss = sorted([client.earnings_per_day() for client in clients])[0]
    if worst_client_loss >= 0:
        return None
    for client in clients:
        if client.earnings_per_day() == worst_client_loss and (
            worst_client is None or client.account_age < worst_client.account_age
        ):
            worst_client = client
    return worst_client
...
if __name__ == "__main__":
    print(
        read_from_file_into_list("clients_info.txt")
    )  # -> [Ann, Mark, Josh, Jonah, Franz]
