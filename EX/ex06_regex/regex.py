...
def find_words_from_sentences_only(text: str) -> list:
    ...
    """
    return re.findall(r'[\w]+', ''.join(find_sentences(text)))
...
def find_years(text: str) -> list:
    ...
    """
    return [int(year) for year in re.findall(r'(?<!\d)\d{4}(?!\d)', text)]
...
def find_phone_numbers(text: str) -> dict:
    ...
    """
    phone_numbers = re.findall(r'(\+\d{3})? ?(\d{7,8})', text)
    result = {}
    for phone_number in phone_numbers:
        if phone_number[0] in result:
            result[phone_number[0]].append(phone_number[1])
        else:
            result[phone_number[0]] = [phone_number[1]]
    return result
...
if __name__ == '__main__':
    ...
    print("find_words_from_sentence:")
