...
def switch_lasts_and_firsts(s: str) -> str:
    ...
    """
    return s[::-1] if len(s) < 4 else s[-2:] + s[2:-2] + s[:2]
...
def take_partial(text: str, leave_count: int, take_count: int) -> str:
    ...
    """
    if leave_count + take_count > len(text):
        return text[leave_count:]
    else:
        return text[leave_count: leave_count + take_count] + take_partial(
            text[leave_count + take_count:], leave_count, take_count
        )
