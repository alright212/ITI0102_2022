...
def sort_tuples_by_length(tuples: list[tuple]) -> list:
    ...
    """
    return sorted(tuples, key=len, reverse=True)
...
def sort_people_by_age_name_height(people: list[Person]) -> list:
    ...
    :return: Sorted list of people.
    """
    return sorted(people, key=lambda person: (person.age, person.name, person.height))
...
def sort_people_by_popularity_of_name(people: list[Person]) -> list:
    ...
    return sorted(
        people,
        key=lambda person: (
            -len(list(filter(lambda x: x.name == person.name, people))),
            person.name,
        ),
    )
...
if __name__ == "__main__":
    ...
    print(
        sort_people_by_age_name_height(people)
    )  # -> [sebastian, alex, lukas2, lukas, ellie]
    print(
        sort_people_by_popularity_of_name(people)
    )  # [lukas, lukas2, alex, ellie, sebastian]
